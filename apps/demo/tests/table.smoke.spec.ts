...
async function findTableWithActionsColumn(
  ...
  for (let index = 0; index < tableCount; index += 1) {
    ...
    const lastHeaderText = await candidate.locator('thead tr').first().locator('th').last().textContent();
...
async function verifyStickyActionsColumn(pagePath: string, tableSelector: string, page: import('@playwright/test').Page) {
  ...
  const scrollContainer = table.locator('xpath=ancestor-or-self::*[contains(@class, "overflow-auto")]').first();
