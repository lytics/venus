...
export async function GET() {
  try {
    ...
    return NextResponse.json(
      {
        error: "Failed to read current theme",
 ...
 */
function extractBrandColors(content: string, selector: ":root" | ".dark"): Record<string, string> {
  // Find the selector block
  const selectorRegex = selector === ":root" ? /:root\s*\{/ : /\.dark\s*\{/;
