...
export async function POST(request: Request) {
  try {
    ...
    if (!Array.isArray(icons) || icons.length === 0) {
      return NextResponse.json({ error: "Invalid icons array" }, { status: 400 });
    }

    // Read the icons page file
    const filePath = path.join(process.cwd(), "src/app/(galleries)/icons/page.tsx");
    ...
    return NextResponse.json({ error: "Failed to update curated list" }, { status: 500 });
  }
}
