          ...
          <Table full className="min-h-full border-separate border-spacing-0">
            <TableHeader>
              <TableRow>
                ...
                <TableHead className="w-12 sticky right-0 bg-white z-10 before:absolute before:left-[-10px] before:top-0 before:bottom-0 before:w-[10px] before:bg-gradient-to-l before:from-black/5 before:to-transparent before:pointer-events-none text-center">
                  Actions
                </TableHead>
              </TableRow>
            </TableHeader>
            <TableBody>
              {filteredAttributes.map((attribute, index) => (
                <TableRow key={index}>
                  ...
                  <TableCell className="text-gray-500">{attribute.key || "-"}</TableCell>
                  <TableCell className="max-w-sm truncate font-medium">
                    {attribute.description}
                  </TableCell>
                  <TableCell className="font-mono text-sm">{attribute.uniqueId}</TableCell>
              ...
              <div className="flex items-center gap-2">
                <input
                  ...
                  onChange={(e) => {
                    const page = Math.max(
                      1,
                      Math.min(Math.ceil(totalRecords / recordsPerPage), Number(e.target.value)),
                    );
                    setCurrentPage(page);
                  }}
