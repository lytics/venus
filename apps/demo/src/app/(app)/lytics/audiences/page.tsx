...
import { SearchV3, Button, Table, TableHeader, TableBody, TableHead, TableRow, TableCell, TableActionButton, Dropdown, StatusPill } from '@contentstack/venuscn'
import { cn } from '@/lib/utils'
import { audiences } from '@/data/lytics-audiences'
...
export default function AudiencesPage() {
  ...
  const handleNewAudience = () => {
    router.push('/lytics/audiences/new')
  }

  const handleRowAction = (audienceName: string) => {
    // TODO: Implement row action for audience
  }

  // Filter audiences based on search
  const filteredAudiences = audiences.filter((audience) =>
    audience.name.toLowerCase().includes(searchValue.toLowerCase()) ||
    audience.description.toLowerCase().includes(searchValue.toLowerCase())
  )

  const totalRecords = filteredAudiences.length
    ...
    <div className="flex flex-col h-full overflow-hidden">
      ...
      <div className="bg-[#F7F9FC] h-[90px] border-b border-gray-200 flex items-center justify-between px-4">
        ...
        <div className="flex items-center gap-7">
          <h1 className="text-xl font-semibold text-black">Audiences</h1>
          <HelpCircle className="w-5 h-5 text-[#697B9B]" strokeWidth={1.5} />
        </div>

        {/* Right: Action button */}
        <Button variant="primary" size="regular" onClick={handleNewAudience} className="h-10">
            ...
            <TableBody>
              {filteredAudiences.map((audience) => (
                <TableRow key={audience.id}>
                  ...
                  <TableCell className="font-medium">{audience.description}</TableCell>
                  <TableCell>{audience.size.toLocaleString()}</TableCell>
                  <TableCell>
                    <StatusPill
                      status={audience.status === 'active' ? 'published' : audience.status === 'draft' ? 'draft' : 'archived'}
                    />
                  </TableCell>
                  <TableCell>{audience.modifiedAt}</TableCell>
                  ...
                  <TableCell className="sticky right-0 bg-white group-hover:bg-[#F9FAFB] z-10 before:absolute before:left-[-10px] before:top-0 before:bottom-0 before:w-[10px] before:bg-gradient-to-l before:from-black/5 before:to-transparent before:pointer-events-none text-center">
                    <TableActionButton
                      onClick={() => handleRowAction(audience.name)}
