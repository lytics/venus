...
/** Lightweight CSS/SVG bar and line charts */
    ...
    <div className="flex items-end gap-2 w-full" style={{ height }}>
      {data.map((point, i) => {
        ...
        return (
          ...
          >
            <span className="text-xs font-medium text-heading tabular-nums">{point.value}</span>
            <div
            ...
            />
            <span className="text-xs text-body truncate w-full text-center" title={point.label}>
              {point.label}
            </span>
          </div>
        );
  ...
  return (
    <div className="flex flex-col" style={{ height }}>
      ...
      <div className="flex justify-between px-4 mt-1">
        {data.map((d, i) => (
          <span key={i} className="text-xs text-body truncate flex-1 text-center" title={d.label}>
            {d.label}
          </span>
        ))}
      </div>
