...
/** Structured list with compound sub-components */
...
export const List = React.forwardRef<HTMLUListElement, ListProps>(
  ({ className, children, ...props }, ref) => (
    <ul ref={ref} className={cn("w-full list-none p-0 m-0", className)} {...props}>
      {children}
    </ul>
  ),
);
...
export const ListItem = React.forwardRef<HTMLLIElement, ListItemProps>(
  ({ icon, itemTitle, description, action, className, children, ...props }, ref) => (
    <li
      ...
      {...props}
    >
      {icon && <span className="shrink-0 flex items-center justify-center text-body">{icon}</span>}
      <span className="flex-1 min-w-0">
        {itemTitle && (
          <span className="block text-sm font-medium text-title truncate">{itemTitle}</span>
        )}
        {description && (
          <span className="block text-xs text-body truncate mt-0.5">{description}</span>
        )}
        {children}
      </span>
