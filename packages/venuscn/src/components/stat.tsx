...
export const Stat = React.forwardRef<HTMLDivElement, StatProps>(
  ({ value, label, change, trend = "neutral", icon, className, ...props }, ref) => {
    const changeColor =
      trend === "up" ? "text-green-600" : trend === "down" ? "text-red-500" : "text-body";
    ...
    return (
      <div
        ...
        {...props}
      >
        {icon && <div className="absolute right-4 top-4 text-body">{icon}</div>}
        <span className="text-[30px] font-bold leading-none text-title">{value}</span>
        <span className="text-sm font-normal text-body">{label}</span>
        {change && <span className={cn("text-sm font-medium", changeColor)}>{change}</span>}
      </div>
    );
  },
);
